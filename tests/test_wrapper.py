...
def test_entry_point_resolves_to_the_real_server():
    import wildberries_mcp_ru

    assert callable(wildberries_mcp_ru.main), "точка входа не вызывается"
    assert wildberries_mcp_ru.main.__module__.startswith("wb_mcp")
...
def test_versions_agree():
    ...
    assert py == sj["version"] == sj["packages"][0]["version"] == wildberries_mcp_ru.__version__
...
def test_readme_number_matches_the_catalog_that_ships():
    ...
    assert len(rows) == 307, "каталог изменился, пересобери обвязку генератором"
    assert f"**{len(rows)} " in readme
    assert counts == {"read": 187, "write": 108, "destructive": 12}
...
def test_skill_description_fits_the_spec():
    ...
    text = (ROOT / "skills" / "wildberries-mcp" / "SKILL.md").read_text(encoding="utf-8")
    head = text.split("---")[1]
